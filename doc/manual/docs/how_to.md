...
ENABLE_MICRO_ECC_P256                      | Enable [micro-ecc library](https://github.com/kmackay/micro-ecc) for ECC-P256 operations
ENABLE_SCO_OVER_HCI                        | Enable SCO over HCI for chipsets (only TI CC256x/WL18xx, CSR + Broadcom H2/USB))

Notes:
- ENABLE_MICRO_ECC_P256: Only some Bluetooth 4.2+ controllers (e.g., EM9304, ESP32) support the necessary HCI commands. Others reasons to enable the ECC software implementations are if the Host is much faster or if the micro-ecc library is already provided (e.g., ESP32, WICED)
